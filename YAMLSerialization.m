...
static void __YAMLMapArray (NSMutableArray *array) {
    for (int i = 0; i < array.count; i++) {
        id object = array[i];
        if ([object isKindOfClass:NSDictionary.class]) {
            id mappedObject = __YAMLMapDictionary (object);
            [array replaceObjectAtIndex:i withObject:mappedObject];
        }
    }
}
    ...
    if (dictionary[@"_yaml_mapClass"]) {
        ...
        for (int i = 0; i < allKeys.count; i++) {
            ...
            if ([value isKindOfClass:NSArray.class]) {
                __YAMLMapArray (value);
            } else if ([value isKindOfClass:NSDictionary.class]) {
                dictionary[key] = __YAMLMapDictionary (value);
            }
        }
        return dictionary;
    }
...
static id
__YAMLSerializationObjectWithYAMLDocument (yaml_document_t *document, NSDictionary<NSString *, Class> *customClasses, YAMLReadOptions opt, NSError **error) {
    ...
    for (node = document->nodes.start, i = 0; node < document->nodes.top; node++, i++) {
        switch (node->type) {
            ...
            case YAML_MAPPING_NODE: {
                ...
                NSInteger capacity = node->data.mapping.pairs.top - node->data.mapping.pairs.start;
                if (_custom_tag) {
                    capacity += 1;
                }
                objects[i] = [[dictionaryClass alloc] initWithCapacity:capacity];

                if (_custom_tag) {
                    Class customClass = customClasses[_custom_tag];
                    if ([customClass conformsToProtocol:@protocol (YAMLSeriazableProtocol)]) {
                        [objects[i] setObject:customClass forKey:@"_yaml_mapClass"];
                    }
                }
                if (!root)
                    root = objects[i];
...
+ (NSMutableArray *)objectsWithYAMLStream:(NSInputStream *)stream
                            customClasses:(NSDictionary<NSString *, Class> *)customClasses
    ...
    while (!done) {
        ...
        if (!done) {
            documentObject = __YAMLSerializationObjectWithYAMLDocument (&document, customClasses, opt, error);
            if (error && *error) {
...
+ (id)objectWithYAMLStream:(NSInputStream *)stream customClasses:(NSDictionary<NSString *, Class> *)customClasses options:(YAMLReadOptions)opt error:(NSError **)error {
    return [[self objectsWithYAMLStream:stream customClasses:customClasses options:opt error:error] objectAtIndex:0];
}
                                  ...
                                  error:(NSError **)error;
{
    ...
    if (data != nil) {
        ...
        result = [self objectsWithYAMLStream:stream customClasses:customClasses options:opt error:error];
        [stream release];
    }
    return result;
}

+ (id)objectWithYAMLData:(NSData *)data customClasses:(NSDictionary<NSString *, Class> *)customClasses options:(YAMLReadOptions)opt error:(NSError **)error {
    return [[self objectsWithYAMLData:data customClasses:customClasses options:opt error:error] objectAtIndex:0];
}
...
+ (id)objectWithYAMLString:(NSString *)string customClasses:(NSDictionary<NSString *, Class> *)customClasses options:(YAMLReadOptions)opt error:(NSError **)error {
    return [[self objectsWithYAMLString:string customClasses:customClasses options:opt error:error] objectAtIndex:0];
}
...
+ (NSMutableArray *)YAMLWithStream:(NSInputStream *)stream options:(YAMLReadOptions)opt error:(NSError **)error {
    return [self objectsWithYAMLStream:stream customClasses:nil options:opt error:error];
}

+ (NSMutableArray *)YAMLWithData:(NSData *)data options:(YAMLReadOptions)opt error:(NSError **)error {
    return [self objectsWithYAMLData:data customClasses:nil options:opt error:error];
}
